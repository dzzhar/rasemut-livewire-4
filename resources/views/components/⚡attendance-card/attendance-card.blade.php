<flux:card size="sm"
    class="flex items-center justify-between bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">

    <div class="flex items-center gap-4">
        <flux:badge :color="$title === 'Check In' ? 'blue' : 'red'"
            class="flex items-center justify-center w-14 h-14 rounded-xl" size="sm">
            <flux:icon :name="$title === 'Check In' ? 'arrow-right-end-on-rectangle' : 'arrow-right-start-on-rectangle'"
                class="size-7" />
        </flux:badge>

        <div>
            <flux:text>{{ $title }}</flux:text>
            <flux:heading size="xl" level="1" class="mt-1 font-semibold">
                {{ filled($time) ? "$time WIB" : '--:--:--' }}
            </flux:heading>
        </div>
    </div>
</flux:card>
